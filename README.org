...
- Jalankan `npm install` atau perintah serupa di folder proyek. Butuh nodejs, deno atau bun tentunya
...
[[./qr-questions-monkey.png][QR Code menuju diskusi online]]
...
Ini level paling atas - kita mendefinisikan ide besarnya. Untuk contoh kali ini, kita representasikan dalam format /product brief/ atau /product description/. Contoh: "Aplikasi web yang menampilkan tanggal dan jam real-time untuk berbagai zona waktu di Indonesia."
...
*** Computational thinking: penjelasan mendetail dari apa yang mau kita buat
...
Keempat lapisan di atas adalah kerangka yang kita adaptasi dari berbagai framework pemecahan masalah (seperti Bloom's taxonomy dan computational thinking) - khusus disederhanakan untuk memudahkan kita ngasih instruksi ke coding agent.
...
Catatan: Kalau sudah punya =.cursorrules= dari versi Cursor sebelumnya, bisa buat folder =.cursor/rules/= lalu pindahkan isinya ke file =.cursor/rules/project.mdc=. Tambahkan metadata YAML di awal file kalau perlu atur =glob= atau =alwaysApply=. Setelah itu, hapus atau rename =.cursorrules= (misal jadi =.cursorrules.bak=)
supaya nggak konflik.
...
- *Apply Intelligently*: Cursor yang memutuskan sendiri kapan aturan ini relevan berdasarkan isi prompt. Fitur ini adalah salah satu fitur pembeda dari tools lain yang umumnya hanya mendukung always-apply atau manual invocation.
...
Beri masukan agar saya bisa lebih baik di kesempatan berikutnya: https://feedback.rizafahmi.com/

#+CAPTION: QR Code untuk masukan
#+NAME: qr-monkey-feedback.png
[[./qr-feedback-monkey.png][QR Code untuk memberi masukan]]

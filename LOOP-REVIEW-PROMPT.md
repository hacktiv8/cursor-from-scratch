...
1. Put the target document path in `{DOC}`.
2. Pick a lens from the **Lens Roster** below and put it in `{LENS}`.
...
TARGET DOCUMENT: {DOC}
THIS RUN'S LENS: {LENS}   (see LOOP-REVIEW-PROMPT.md "Lens Roster" for the exact

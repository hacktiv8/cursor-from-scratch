...
Kalau ingin mengikuti materi ini, pastikan teman-temam mempersiapkan hal-hal berikut:
...
Silakan interupsi kapan saja jika ada kendala, topik diskusi atau bertanya. Sesi ini didesain agar interaktif dan mengalir dua arah. Ajukan topik diskusi atau pertanyaaan ke: https://feedback.rizafahmi.com/questions
...
- *Apply to Specific Files*: Aturan cuma berlaku untuk file yang cocok dengan =globs= pattern (misal =*.tsx= untuk file React, atau =*.test.ts= untuk file testing).
...
Highlight dan buat zona waktunya menjadi lebih besar
...
printf '%s\n' '{"command":"npm install moment"}' \
        | .cursor/hooks/guard-npm.sh
{"permission":"deny","agentMessage":"Blocked: adding new dependencies is not allowed. Use existing packages or built-in APIs like Intl.DateTimeFormat."}
#+END_SRC

Menguji hook yang akan memperbolehkan perintah `npm test`.
...
Sekarang mari kita mencobanya langsung di coding agent dengan mengeksekusi perintah berikut. Kalau hook sudah terpasang dengan benar, agent akan mencoba =npm install moment= dan hook akan memblokirnya dengan peringatan seperti di screenshot bawah.
...
#+END_SRC
 ...
 ⚠️ Sengaja kita pakai =moment.js= di prompt di atas untuk memicu hook =guard-npm.sh=. Sebaiknya =Intl.DateTimeFormat= bawaan JavaScript jika ingin berinteraksi dengan tanggal dan jam.
...
#+CAPTION: Contoh Customization Marketplace - kita bisa cari dan instal kustomisasi buatan komunitas seperti Continual Learning, Superpower, atau Compound Engineering
...
- *Globs Pattern*: Pola pencocokan nama file pakai wildcard - misalnya =*.tsx= cocok untuk semua file berakhiran .tsx.
...
- =.mdc= (Markdown for Cursor): Format markdown khusus Cursor dengan dukungan metadata seperti =globs= dan =alwaysApply=.
...
- *npm (Node Package Manager)*: Tool untuk meng-instal dan mengelola library JavaScript. Dipakai lewat perintah =npm install=.

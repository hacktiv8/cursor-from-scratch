...
2. Put the target doc in `{DOC}`, the findings log path in `{FINDINGS_FILE}`,
   the doc format in `{FORMAT}`, and the batch scope in `{BATCH}`.
3. Run the prompt once per batch. `loop.sh --phase fix` fills the placeholders
   and picks `{BATCH}` for you (next open severity tier by default).
...
TARGET DOCUMENT: {DOC}
FINDINGS LOG:    {FINDINGS_FILE}   (this is your work queue — read it first)
DOC FORMAT:      {FORMAT}
THIS BATCH:      {BATCH}
...
- PRESERVE FORMAT SYNTAX ({FORMAT}). Keep code fences, links, captions,
